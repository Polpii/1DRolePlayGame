...
function keyPressed() {
    ...
    if (!audioUnlocked) {
        ...
        // Start the GO sound immediately if we are in GO.
    ...
    if (key == 'D' || key == 'd') {
        if (playerActive[0] && !playerOne.eliminated && (controller.gameState === "PLAY" || controller.gameState === "VOTE")) {
            if (controller.gameState === "PLAY" && playerOne.position >= controller.lightIndex) return;
            if (controller.gameState === "VOTE") {
                controller.moveVotePlayer(playerOne, 1);
                controller.noteMotionForPlayer(playerOne);
            } else if (controller.canMoveNow()) {
                playerOne.move(1);
                controller.noteMotionForPlayer(playerOne);
                controller.noteStepForPlayer(playerOne);
            } else {
                if (playerOne === controller.getGameMasterPlayer()) {
                    // GM can't move on red, but can 'rebouge' to re-enable cyan
                    controller.noteMotionForPlayer(playerOne);
                } else if (controller.isStopGraceActive && controller.isStopGraceActive()) {
                    // grace period: allow movement without elimination
                    playerOne.move(1);
                } else {
                    controller.eliminatePlayer(playerOne);
                }
            }
        }
    }
    ...
    if (key == 'L' || key == 'l' || key === "ArrowRight") {
        if (playerActive[1] && !playerTwo.eliminated && (controller.gameState === "PLAY" || controller.gameState === "VOTE")) {
            if (controller.gameState === "PLAY" && playerTwo.position >= controller.lightIndex) return;
            if (controller.gameState === "VOTE") {
                controller.moveVotePlayer(playerTwo, 1);
                controller.noteMotionForPlayer(playerTwo);
            } else if (controller.canMoveNow()) {
                playerTwo.move(1);
                controller.noteMotionForPlayer(playerTwo);
                controller.noteStepForPlayer(playerTwo);
            } else {
                if (playerTwo === controller.getGameMasterPlayer()) {
                    controller.noteMotionForPlayer(playerTwo);
                } else if (controller.isStopGraceActive && controller.isStopGraceActive()) {
                    // grace period: allow movement without elimination
                    playerTwo.move(1);
                } else {
                    controller.eliminatePlayer(playerTwo);
                }
            }
        }
    }

    // Player 3 (Red) advances with O
    if (key == 'O' || key == 'o') {
        if (playerActive[2] && typeof playerThree !== 'undefined' && playerThree && !playerThree.eliminated && (controller.gameState === "PLAY" || controller.gameState === "VOTE")) {
            if (controller.gameState === "PLAY" && playerThree.position >= controller.lightIndex) return;
            if (controller.gameState === "VOTE") {
                controller.moveVotePlayer(playerThree, 1);
                controller.noteMotionForPlayer(playerThree);
            } else if (controller.canMoveNow()) {
                playerThree.move(1);
                controller.noteMotionForPlayer(playerThree);
                controller.noteStepForPlayer(playerThree);
            } else {
                if (playerThree === controller.getGameMasterPlayer()) {
                    controller.noteMotionForPlayer(playerThree);
                } else if (controller.isStopGraceActive && controller.isStopGraceActive()) {
                    // grace period: allow movement without elimination
                    playerThree.move(1);
                } else {
                    controller.eliminatePlayer(playerThree);
                }
            }
        }
    }

    // Player 4 (Yellow) advances with J
    if (key == 'J' || key == 'j') {
        if (playerActive[3] && typeof playerFour !== 'undefined' && playerFour && !playerFour.eliminated && (controller.gameState === "PLAY" || controller.gameState === "VOTE")) {
            if (controller.gameState === "PLAY" && playerFour.position >= controller.lightIndex) return;
            if (controller.gameState === "VOTE") {
                controller.moveVotePlayer(playerFour, 1);
                controller.noteMotionForPlayer(playerFour);
            } else if (controller.canMoveNow()) {
                playerFour.move(1);
                controller.noteMotionForPlayer(playerFour);
                controller.noteStepForPlayer(playerFour);
            } else {
                if (playerFour === controller.getGameMasterPlayer()) {
                    controller.noteMotionForPlayer(playerFour);
                } else if (controller.isStopGraceActive && controller.isStopGraceActive()) {
                    // grace period: allow movement without elimination
                    playerFour.move(1);
                } else {
                    controller.eliminatePlayer(playerFour);
                }
            }
        }
    }

    // Player 5 (Green) advances with V
    if (key == 'V' || key == 'v') {
        if (playerActive[4] && typeof playerFive !== 'undefined' && playerFive && !playerFive.eliminated && (controller.gameState === "PLAY" || controller.gameState === "VOTE")) {
            if (controller.gameState === "PLAY" && playerFive.position >= controller.lightIndex) return;
            if (controller.gameState === "VOTE") {
                controller.moveVotePlayer(playerFive, 1);
                controller.noteMotionForPlayer(playerFive);
            } else if (controller.canMoveNow()) {
                playerFive.move(1);
                controller.noteMotionForPlayer(playerFive);
                controller.noteStepForPlayer(playerFive);
            } else {
                if (playerFive === controller.getGameMasterPlayer()) {
                    controller.noteMotionForPlayer(playerFive);
                } else if (controller.isStopGraceActive && controller.isStopGraceActive()) {
                    // grace period: allow movement without elimination
                    playerFive.move(1);
                } else {
                    controller.eliminatePlayer(playerFive);
                }
            }
        }
    }
